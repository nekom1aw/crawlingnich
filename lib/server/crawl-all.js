...
const { fetchArticleForUrl } = require("./preview");
...
function buildNewsFeedUrls(variant = "") {
  ...
  return [
    ...internationalQueries.flatMap((internationalQuery) => [
      `https://www.bing.com/news/search?q=${encodeURIComponent(internationalQuery)}&format=rss&setlang=en-US`,
      `https://news.google.com/rss/search?q=${encodeURIComponent(internationalQuery)}&hl=en-US&gl=US&ceid=US:en`,
    ]),
...
async function crawlKeywordPair({ primary, secondary, startBoundary, endBoundary }) {
  ...
  try {
    ...
    for (const feedResult of feedResponses) {
      if (addedNews >= NEWS_RESULT_LIMIT) break;
  ...
  try {
    ...
    for (const item of webResults) {
      if (!item.link || seen.has(item.link) || !isUsableArticleUrl(item.link)) continue;
      seen.add(item.link);
      pairResults.push({
        type: "news",
        title: item.title,

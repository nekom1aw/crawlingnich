...
function buildSummaryPrompt(query, articles) {
  const articleLimit = Number(process.env.AI_SUMMARY_ARTICLE_LIMIT || 12);
  ...
  while (true) {
    ...
    for (const line of lines) {
      ...
      try {
        ...
        // Ignore partial/heartbeat chunks.
      }
    }
  }
...
async function summarizeNewsWithNvidiaGLM(query, articles) {
  ...
  const useStream = process.env.NVIDIA_STREAM !== "0";
  const response = await fetch(`${baseUrl}/chat/completions`, {
    ...
    signal: AbortSignal.timeout(Number(process.env.NVIDIA_TIMEOUT_MS || 240000)),
  });

  if (!response.ok) {
    const data = await response.json().catch(() => ({}));
    throw new Error(data.error?.message || `NVIDIA GLM request failed (${response.status})`);
  }

  const summary = useStream
    ? await readOpenAICompatibleStream(response)
    : extractChatCompletionText(await response.json().catch(() => ({})));
...
async function summarizeNewsWithAI(query, articles) {
  ...
  try {
    ...
    providerErrors.push(`NVIDIA GLM gagal: ${err.message}`);
  }
  ...
  try {
    ...
    providerErrors.push(`Vercel AI Gateway gagal: ${err.message}`);
  }
  ...
  try {
    ...
    providerErrors.push(`Direct Gemini SDK gagal: ${err.message}`);
  }
  ...
  if (!apiKey) {
    try {
      ...
      providerErrors.push(`Gemini gagal: ${err.message}`);
    }
    ...
    return {
      ...
      summary: buildFallbackNewsSummary(
        ...
        providerErrors.length
          ? `Mode fallback dipakai karena provider AI gagal: ${providerErrors.join(" | ")}`
          : "Mode fallback dipakai karena belum ada provider AI yang aktif di server."
      ),
    };
  }

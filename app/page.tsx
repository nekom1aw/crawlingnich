...
.btn-crawl.is-cancel {
  background: linear-gradient(180deg, #ff5b5b, #d92f2f);
  border-color: rgba(180, 30, 30, 0.38);
  color: #fff;
  box-shadow: 0 12px 28px rgba(210, 47, 47, 0.22);
}
.btn-crawl.is-cancel:hover { background: linear-gradient(180deg, #ff6b6b, #c92323); }
  ...
  <div className="shell">
    <div className="main is-preview-collapsed">
      ...
      <aside className="filter-sidebar">
        ...
        <div>
          <div className="section-label">Secondary Keyword <span className="optional-label">Opsional</span></div>
          <div className="keyword-group">
            <div className="input-wrap secondary-tag">
              <i className="ti ti-tag input-icon" />
              <input type="text" id="secondaryInput" placeholder="boleh dikosongkan..." />
            </div>
      ...
      <div className="content">
        ...
        <div className="stats-bar">
          ...
          <div className="stats-actions">
            <button className="btn-ai-summary" id="summarizeCrawlBtn" type="button" disabled><i className="ti ti-sparkles" /> Ringkas AI</button>
...
html[data-theme="dark"] .btn-ai-summary {
  border-color: #f4f4f4;
  background: #f4f4f4;
  color: #000;
}
...
function getCrawlSummaryQuery(rows = []) {
  const primary = getTags('primaryTags');
  const secondary = getTags('secondaryTags');
  const keywordText = [...primary, ...secondary].filter(Boolean).join(', ');
  const fromDate = document.getElementById('fromDate')?.value || '';
  const toDate = document.getElementById('toDate')?.value || '';
  const rangeText = fromDate || toDate ? ' periode ' + (fromDate || 'awal') + ' sampai ' + (toDate || 'akhir') : '';
  return 'Ringkas dan analisis hasil crawling berita' + (keywordText ? ' tentang ' + keywordText : '') + rangeText + ' secara fleksibel. Ambil insight utama, pola isu, sumber dominan, dan catatan penting dari data yang tersedia.';
}

function buildCrawlSummaryPackage() {
  const rows = getFiltered()
  ...
  return {
    createdAt: Date.now(),
    query: getCrawlSummaryQuery(rows),
    total: rows.length,
    filters: {
      primaryKeywords: getTags('primaryTags'),
      secondaryKeywords: getTags('secondaryTags'),
      startDate: document.getElementById('fromDate')?.value || '',
      endDate: document.getElementById('toDate')?.value || '',
      activeType,
      search: document.getElementById('searchInResults')?.value || '',
    },
    items: rows,
  };
}

function updateSummaryButton(filtered = getFiltered()) {
  const btn = document.getElementById('summarizeCrawlBtn');
  if (!btn) return;
  const summarizableCount = filtered.filter((item) => item.type !== 'journal').length;
  btn.disabled = !summarizableCount || isLoading;
}

const originalUpdateCountersForSummary = updateCounters;
updateCounters = function updateCountersWithSummary(filtered) {
  originalUpdateCountersForSummary(filtered);
  updateSummaryButton(filtered);
};

function openCrawlSummary() {
  const pkg = buildCrawlSummaryPackage();
  if (!pkg.items.length) {
    alert('Belum ada berita hasil crawling untuk diringkas.');
    return;
  }

  try {
    sessionStorage.setItem(CRAWL_SUMMARY_STORAGE_KEY, JSON.stringify(pkg));
    window.location.href = '/summarized-ai?from=crawl';
  } catch (err) {
    alert('Gagal mengirim hasil crawling ke Summarized AI: ' + err.message);
  }
}

document.getElementById('summarizeCrawlBtn')?.addEventListener('click', openCrawlSummary);
...
const HERO_ICON_PATHS = {
  ...
  sparkles: '<path d="M12 3l1.8 4.7L18.5 9.5l-4.7 1.8L12 16l-1.8-4.7-4.7-1.8 4.7-1.8L12 3Z"/><path d="M19 15l.8 2.2L22 18l-2.2.8L19 21l-.8-2.2L16 18l2.2-.8L19 15Z"/><path d="M5 14l.7 1.8L7.5 16.5l-1.8.7L5 19l-.7-1.8-1.8-.7 1.8-.7L5 14Z"/>',
